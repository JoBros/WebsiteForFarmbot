      ...
      for($i=0; $i<=$tbl_length; $i++) {
        ...
        for($j=0; $j<=$tbl_width; $j++) {
          if($i==0  && $j == 0){
            ...
            echo"<td id='$j.$i' class='anzeigeStatus2'> <img src='seamless-dirt-texture.svg' style=\"width: 40px; height: 40px;\"></td>";
          }
        }
          echo "</tr>";
      }
      if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          if( strcmp($row["bez"], "Radieschen") == 0) {
            echo "document.getElementById('" . $row["x"] . "." . $row["y"] . "').innerHTML = '<img src=\"architetto-ravanello.svg\" style=\"width: 40px; height: 40px;\">';";
          }
        } echo "</script>";
      } else {
